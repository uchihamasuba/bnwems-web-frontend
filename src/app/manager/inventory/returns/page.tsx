...
// Trang thuần giao diện — mirror 1:1 từ src/app/admin/inventory/returns/page.tsx cho vai trò Manager
// (xem giải thích nghiệp vụ ở đầu src/mocks/adminInventoryReturnsMock.ts). Đáp ứng checklist "Thu hồi
// & kiểm đếm" (ghi nhận hỏng/mất) + "Hoàn kho" — Admin và Manager dùng chung 1 bộ mock/service.
...
export default function Page() {
  ...
  const handleCreated = (slip: ReturnSlip) => {
    ...
    router.push(`/manager/inventory/returns/${slip.id}`);
  };
  ...
  const columns: TableColumn<ReturnSlip>[] = [
    {
      ...
      label: 'Mã phiếu',
      render: (slip) => (
        <Link href={`/manager/inventory/returns/${slip.id}`} className="font-semibold text-blue-600 hover:underline">
          #{slip.id}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (slip) => (
        <div className="flex items-center gap-1">
          <Link
            href={`/manager/inventory/returns/${slip.id}`}
