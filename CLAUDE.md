...
- **[Cập nhật sau lần đồng bộ docs/api từ backend `feature/align-new-api-contracts-and-test`]** Mọi vô hiệu hóa/đổi trạng thái giờ dùng **`PATCH .../status`** (đảo ngược hẳn quy ước cũ — trước đây dự án dùng `PUT .../status`, vd `PATCH /users/:id/status`, `PATCH /equipment/:id/status`, `PATCH /quotations/:id/status`, `PATCH /tasks/:id/status`). Hệ thống **không còn dùng method `DELETE` ở bất kỳ đâu** — 2 endpoint `DELETE` từng tồn tại (`DELETE /quotations/:id`, `DELETE /tasks/:id`) đã bị gỡ bỏ hoàn toàn trong lần rewrite này, thay bằng `PATCH .../status` (soft-delete). Nếu thấy code cũ còn gọi `api.delete(...)` cho 2 endpoint này, đó là code chưa migrate.
- Ngày: `YYYY-MM-DD`; thời gian: ISO-8601 UTC; tiền: kiểu `number`, đơn vị VNĐ.
- Mã `code` dạng `MSG-XX-NN` với `XX` là prefix 2 chữ cái theo module (không phải theo số UC như trước) — xem bảng đầy đủ ở [docs/api/README.md](docs/api/README.md) mục A.2: `AU` Auth, `US` User & Role, `CT` Catalog/Equipment, `SP` Supplier, `WH` Warehouse & Inventory, `PO` Policy, `CU` Customer, `QO` Quotation, `CO` Order, `SV` Survey & Assignment, `PM` Payment & Settlement, `MO` Mobile, `RP` Report, `UF` Upload File.
- Nhiều enum trạng thái đổi từ UPPERCASE sang **lowercase** trong lần rewrite này (Order status, User status, Quotation status, Equipment status...) — kiểm tra kỹ giá trị mẫu trong doc module trước khi hardcode so sánh string, đừng giả định theo quy ước cũ.
- `docs/api/README.md` hiện đã được đồng bộ và về cơ bản khớp với các file module (không còn là "mirror lỗi thời" như trước) — vẫn còn 1 điểm lệch nhỏ: ví dụ phân trang ở mục A.4 của README dùng `total`/`total_pages` nhưng mọi file module thực tế đều trả `meta: { page, limit, totalCount }` — **ưu tiên `totalCount` như trong từng file module cụ thể.**
- ⚠️ Backend hiện có 2 tài liệu song song cho danh mục thiết bị: [03-catalog.md](docs/api/03-catalog.md) (entity `Equipment`, đầy đủ schema, coi là chuẩn) và [15-catalog-categories-items.md](docs/api/15-catalog-categories-items.md) ("Legacy — Frontend Compatibility", giữ path `/catalog-items` cũ nhưng không có schema, và không nằm trong mục lục chính thức ở README). FE đã migrate sang dùng `/equipment` trực tiếp (xem `types/equipment.ts`, `services/equipment.service.ts`) theo đúng hướng dẫn "Equipment là bảng đổi tên từ CatalogItem".
